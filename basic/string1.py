...
def donuts(count):
  ...
  if count >= 10:
    ...
    result = count
  return 'Number of donuts: ' + str(result) ## Converting result to string because numbers can't combine
...
def both_ends(s):
  ...
  if len(s) <= 2:
    ...
    result = s[0:2] + s[-2:] ## s[0:2] gets the first and second letter and omits the third letter (index 2); s[-2:] gets the second-to-last letter up until the end.
  return result
